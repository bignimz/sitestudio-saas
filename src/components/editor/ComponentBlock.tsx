...
export default function ComponentBlock({ data, onUpdate, onDelete }: ComponentBlockProps) {
  ...
  return (
    <motion.div
      ref={drag}
      className={`relative group border-2 rounded-lg transition-all ${isDragging ? "opacity-50" : "opacity-100"} ${
    ...
    >
      ...
      {/* Component Content */}
      <div className={isEditing ? "ring-2 ring-blue-500 ring-opacity-50 rounded-lg" : ""}>{renderComponent()}</div>
    </motion.div>
  );
}
