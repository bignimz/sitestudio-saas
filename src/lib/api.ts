...
export const userApi = {
  ...
  async updateProfile(updates: Partial<User>): Promise<ApiResponse<User>> {
    const { data, error } = await supabase.from("users").update(updates).select().single();
  ...
  async createFromUrl(siteUrl: string): Promise<ApiResponse<Project>> {
    try {
      ...
      if (components.length > 0) {
        const { error: componentsError } = await supabase.from("components").insert(components);
  ...
  async reorderComponents(components: Array<{ id: string; position: number }>): Promise<ApiResponse<boolean>> {
    try {
      const updates = components.map(({ id, position }) =>
        supabase.from("components").update({ position }).eq("id", id)
      );
